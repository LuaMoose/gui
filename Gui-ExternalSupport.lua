...
local function lowerTable(t)
	...
	for index, value in pairs(t) do
		Table[string.lower(index)] = value
	end	
	return Table
end
...
local library = {design = getgenv().design == "kali" and "kali" or "uwuware", tabs = {}, draggable = true, flags = {["Menu Accent Color"] = Color3.new(0.5996236205101013,0.4471152424812317,0.971744179725647)}, title = "Starlight v1.052 Beta - Testing", open = false, popup = nil, instances = {}, connections = {}, options = {}, notifications = {}, tabSize = 0, theme = {}, foldername = "Starlight", fileext = ".json"}
...
function library:Tab(title)
    ...
    function tab:AddColumn()
        ...
        function column:Section(title)
            local section = {title = tostring(title.Name), options = {}, canInit = true, column = self}
            ...
            function section:Toggle(option)
                option = lowerTable(option)
                option.section = self
                option.text = tostring(option.name)
                option.state = option.currentvalue or false
            ...
            function section:Button(option)
				option = lowerTable(option)
                option.section = self
                option.text = tostring(option.name)
                option.callback = typeof(option.callback) == "function" and option.callback or function() end
            ...
            function section:Slider(option)
				option = lowerTable(option)
                option.section = self
                option.Text = tostring(option.Text)
